...
# 1D CNN layers for feature extraction from each frame
model.add(TimeDistributed(Conv1D(64, 3, activation='relu', padding='same'), input_shape=(frames, 126, 1)))
...
# Save model
model.save('cnn_lstm_model_20_modified.h5')

import json

# Save training history
with open('training_metrics_20_modified.json', 'w') as f:
...
plt.tight_layout()
plt.savefig('training_graphs_20_m.png')  # Save as image
...
plt.savefig('confusion_matrix_20_m.png')
plt.close()
...
plt.savefig('classification_report_20_m.png')
plt.close()
